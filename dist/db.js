...
exports.saveTelegramSession = saveTelegramSession;
...
exports.getInvestedByTrader = getInvestedByTrader;
exports.saveCopyTradingEvent = saveCopyTradingEvent;
exports.getCopyTradingEvents = getCopyTradingEvents;
exports.getUniqueTraders = getUniqueTraders;
exports.snapshotExistsNearTimestamp = snapshotExistsNearTimestamp;
...
const DB_PATH = process.env.DATABASE_PATH || path_1.default.join(process.cwd(), 'portfolio.db');
...
const dbExec = (0, util_1.promisify)(db.exec.bind(db));
// Initialize database
dbExec(`
...
async function getSetting(key) {
    try {
        const result = await dbGet('SELECT value FROM app_settings WHERE key = ?', [key]);
...
async function saveSetting(key, value) {
    return new Promise((resolve) => {
        db.run(`INSERT INTO app_settings (key, value, updated_at)
       VALUES (?, ?, ?)
       ON CONFLICT(key) DO UPDATE SET value = ?, updated_at = ?`, [key, value, new Date().toISOString(), value, new Date().toISOString()], (err) => {
            if (err) {
...
async function savePortfolio(snapshot, positions) {
    return new Promise((resolve, reject) => {
        db.serialize(() => {
            // Insert snapshot
            db.run(`INSERT INTO portfolio_snapshots 
...
async function getLatestSnapshot() {
    try {
        const snapshot = await dbGet(`SELECT * FROM portfolio_snapshots 
      ...
      WHERE snapshot_id = ?
      ORDER BY market_question`, [snapshot.id]);
...
async function getHistory(limit = 50) {
    try {
        return await dbAll(`SELECT * FROM portfolio_snapshots 
      ORDER BY timestamp DESC 
      LIMIT ?`, [limit]);
    }
    catch (error) {
...
async function getBalanceHistory(range = '7d') {
    try {
        const startTime = getStartTimeForRange(range);
        if (startTime) {
            return await dbAll(`SELECT timestamp, total_balance, invested
        FROM portfolio_snapshots
        WHERE timestamp >= ?
        ORDER BY timestamp ASC`, [startTime.toISOString()]);
        }
        else {
            // 'all' - no time filter
            return await dbAll(`SELECT timestamp, total_balance, invested
...
async function getInvestedByTrader(range = '7d') {
    try {
        const startTime = getStartTimeForRange(range);
        ...
        ORDER BY s.timestamp ASC`, [startTime.toISOString()]);
        }
        else {
            // 'all' - no time filter
            return await dbAll(`SELECT
...
async function saveCopyTradingEvent(event) {
    return new Promise((resolve, reject) => {
        db.run(`INSERT INTO copy_trading_events (timestamp, event_type, description, trader_name)
      VALUES (?, ?, ?, ?)`, [event.timestamp, event.event_type, event.description, event.trader_name || null], function (err) {
            if (err) {
                reject(err);
            }
            else {
...
async function getCopyTradingEvents(limit = 100) {
    try {
        return await dbAll(`SELECT * FROM copy_trading_events 
      ORDER BY timestamp ASC 
      LIMIT ?`, [limit]);
    }
    catch (error) {
...
async function getUniqueTraders(snapshotId) {
    try {
        const positions = await dbAll(`SELECT DISTINCT copied_from FROM positions 
      WHERE snapshot_id = ? AND copied_from IS NOT NULL AND copied_from != ''`, [snapshotId]);
...
async function snapshotExistsNearTimestamp(timestamp, toleranceMinutes = 5) {
    try {
        // Check if a snapshot exists within toleranceMinutes of this timestamp (to avoid duplicates)
        const snapshot = await dbGet(`SELECT id FROM portfolio_snapshots 
      WHERE ABS(julianday(timestamp) - julianday(?)) * 24 * 60 < ?
      LIMIT 1`, [timestamp, toleranceMinutes]);
        return !!snapshot;
    }
    catch (error) {
