...
const auth_web_1 = require("./auth-web");
...
let lastKnownTraders = new Set();
// Initialize lastKnownTraders from the latest snapshot
async function initializeCopyTradingState() {
    try {
        const latest = await (0, db_1.getLatestSnapshot)();
        if (latest && latest.id) {
            const traders = await (0, db_1.getUniqueTraders)(latest.id);
...
async function detectCopyTradingChanges(snapshotId, positions) {
    try {
        ...
        if (lastKnownTraders.size > 0) {
            ...
            const snapshot = await (0, db_1.getLatestSnapshot)();
            if (snapshot) {
                for (const trader of added) {
                    await (0, db_1.saveCopyTradingEvent)({
...
async function initializeTelegram() {
    ...
    const dbSession = await (0, db_1.getTelegramSession)();
    ...
    try {
        telegramClient = new telegram_1.TelegramPortfolioClient(API_ID, API_HASH, currentSession, currentBot);
...
app.post('/api/refresh', async (req, res) => {
    try {
        ...
        const parsed = (0, parser_1.parsePortfolioResponse)(responseText);
        ...
        const snapshotId = await (0, db_1.savePortfolio)(parsed.snapshot, parsed.positions);
...
app.get('/api/portfolio/latest', async (req, res) => {
    try {
        const portfolio = await (0, db_1.getLatestSnapshot)();
        if (!portfolio) {
...
app.get('/api/portfolio/history', async (req, res) => {
    try {
        ...
        const history = await (0, db_1.getHistory)(limit);
        res.json(history);
    }
    catch (error) {
...
app.get('/api/portfolio/balance-history', async (req, res) => {
    try {
        ...
        const history = await (0, db_1.getBalanceHistory)(range);
        const events = await (0, db_1.getCopyTradingEvents)(100);
        const investedByTrader = await (0, db_1.getInvestedByTrader)(range);
...
app.post('/api/historical/fetch', async (req, res) => {
    try {
        ...
        const limit = parseInt(req.body.limit) || 2000;
        ...
        for (const msg of messages) {
            try {
                ...
                const exists = await (0, db_1.snapshotExistsNearTimestamp)(msg.date.toISOString(), 5);
                if (exists) {
                    skipped++;
                    continue;
                }
                // Parse the message
                const parsed = (0, parser_1.parsePortfolioResponse)(msg.message, msg.date);
                ...
                if (parsed.snapshot.total_balance > 0 || parsed.positions.length > 0 || parsed.snapshot.total_positions) {
                    await (0, db_1.savePortfolio)(parsed.snapshot, parsed.positions);
                    saved++;
                    if (saved % 10 === 0) {
...
app.post('/api/auth/start', (req, res) => {
    ...
    try {
        ...
        (0, auth_web_1.createAuthSession)(sessionId, currentApiId, currentApiHash);
...
app.post('/api/auth/phone', async (req, res) => {
    ...
    const result = await (0, auth_web_1.submitPhoneNumber)(sessionId, phoneNumber);
    res.json(result);
});
app.post('/api/auth/code', async (req, res) => {
    ...
    const result = await (0, auth_web_1.submitCode)(sessionId, code);
    if (result.success && result.sessionString) {
        // Save session string to database (secure, not exposed to frontend)
        await (0, db_1.saveTelegramSession)(result.sessionString);
...
app.post('/api/auth/password', async (req, res) => {
    ...
    const result = await (0, auth_web_1.submitPassword)(sessionId, password);
...
app.get('/auth', (req, res) => {
    res.sendFile(path_1.default.join(__dirname, '../public/auth.html'));
});
// Serve dashboard
app.get('/', (req, res) => {
    res.sendFile(path_1.default.join(__dirname, '../public/view.html'));
});
